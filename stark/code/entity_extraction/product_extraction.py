...
def process_product_extraction_response(response_str: str) -> List[str]:
    ...
    try:
        ...
        if isinstance(result, dict) and not ('result' in result and isinstance(result['result'], list)):
            ...
            # Define noise words to filter out
            noise_words = {
                "full refund", "free shipping", "money back", "best quality", 
                "high quality", "value pack", "limited time", "satisfaction guaranteed",
                "restrictions apply"
            }
                ...
                if isinstance(entities, list):
                    ...
                    for entity in entities:
                        if isinstance(entity, str):
                            ...
                            if entity_lower in noise_words or entity_text in seen_in_cat:
                                continue
                    ...
                    if entity_text:
                        entity_lower = entity_text.lower()
                        if entity_lower not in noise_words:
                            if not (',' in entity_text or
                                    ' and ' in entity_lower or
                                    ' with ' in entity_lower or
                                    ' or ' in entity_lower or
                ...
                if isinstance(item, dict) and 'entity' in item and 'category' in item:
                    ...
                    if entity_text and category:
                        ...
                        if not (',' in entity_text or
                            ...
                            if entity_text not in categorized_entities[category]:
...
5. Avoid extracting generic adjectives or marketing vocabulary. **Strictly prohibit** extracting promotional or service-related terms such as "Full Refund", "Free Shipping", "Money Back Guarantee", "Satisfaction Guaranteed", "Value Pack", etc.
...
8. **Digital File Formats**: For embroidery designs or digital products, place all specific file formats (e.g., "PES", "JEF", "EXP", "XXX", "VIP", "HUS", "DST") exclusively under the **"Accessories"** category.
9. **Dimensions and Quantity**: Ensure extracted dimensions and quantities are unique and concise. Do not repeat the same measurement in different units or duplicate them in the output arrays.
10. **Color Entities Must Be Single Colors (Color = One Color Only)**:
