...
class AmazonSKB(SKB):
    ...
    REVIEW_CATEGORIES = set([
        ...
        'Prime_Pantry', 'Software', 'Tools_and_Home_Improvement',
    ...
    QA_CATEGORIES = set([
        ...
        'Musical_Instruments', 'Office_Products', 'Patio_Lawn_and_Garden', 'Pet_Supplies',
        'Arts_Crafts_and_Sewing', 'Tools_and_Home_Improvement', 'Toys_and_Games', 'Video_Games'
    ])
    ...
    def __init__(self, 
                 ...
                 categories: list = ['Arts_Crafts_and_Sewing'], 
        ...
        # Always ensure sub_category_path is set
        if (self.root is None) or (
            ...
            if download_processed:
                ...
                # For local processing, skip downloading category_list.json
                # Use a default path or None
                self.sub_category_path = None
        else:
            self.sub_category_path = osp.join(self.root, "category_list.json") if self.root is not None else None

        # Check if cache exists using the new cache key format
        cache_key = '-'.join(sorted(categories)) + '__' + '-'.join(meta_link_types) if categories and meta_link_types else '-'.join(meta_link_types or [])
        expected_cache_path = osp.join(self.root, "processed", "cache", cache_key) if self.root else None
        cache_exists = expected_cache_path and osp.exists(osp.join(expected_cache_path, "node_info.pkl"))

        # COMMENTED OUT: Entire download branch disabled for debugging
        # if download_processed and ((self.root is None) or (
        #         self.root is not None
        #         and meta_link_types is not None
        #         and not cache_exists
        #     )):
        #     print("DEBUG: Will download processed data from HuggingFace")
        #     print(f"DEBUG: Starting HF download...")
        ...
        #         # For HF cache mode, set directories to point to extracted location
        #         self.processed_data_dir = osp.join(self.root, "processed")
        #         self.raw_data_dir = osp.join(self.root, "raw")
        #         print(f"DEBUG: Set self.root = {self.root}")
        #         print(f"DEBUG: Set self.processed_data_dir = {self.processed_data_dir}")
        #
        #     # Use the same cache key format as the actual cache path
        #     cache_key = '-'.join(sorted(categories)) + '__' + '-'.join(meta_link_types)
        #     expected_cache_file = osp.join(
        #         self.root,
        #         "processed",
        #         "cache",
        #         cache_key,
        #         "node_info.pkl",
        #     )
        #     print(f"DEBUG: Checking if cache exists: {expected_cache_file}")
        #     print(f"DEBUG: Cache exists: {osp.exists(expected_cache_file)}")
        #
        #     if not osp.exists(expected_cache_file):
        #         print(f"DEBUG: Cache not found, extracting zip...")
        ...
        #
        #         # After extraction, ensure processed_data_dir exists
        #         if not osp.exists(self.processed_data_dir):
        #         os.makedirs(self.processed_data_dir, exist_ok=True)
        #             print(f"DEBUG: Created processed_data_dir: {self.processed_data_dir}")
        #         else:
        #             print(f"DEBUG: processed_data_dir already exists: {self.processed_data_dir}")
        #     else:
        #             print(f"DEBUG: Cache already exists, skipping extraction")

        # Create cache path that includes both categories and meta_link_types to avoid conflicts
        cache_key = '-'.join(sorted(categories)) + '__' + '-'.join(meta_link_types)
        cache_path = None if meta_link_types is None or self.processed_data_dir is None else osp.join(self.processed_data_dir, 'cache', cache_key)
    ...
    def _process_raw(self, categories: list) -> dict:
        ...
        print(f'DEBUG: review_categories={review_categories}')
        ...
        print('DEBUG: Data downloading check completed')
        print(f'DEBUG: processed_data_dir={self.processed_data_dir}')
        print(f'DEBUG: node_info.pkl exists: {osp.exists(osp.join(self.processed_data_dir, "node_info.pkl"))}')

        if self.processed_data_dir is None or not osp.exists(osp.join(self.processed_data_dir, 'node_info.pkl')):
            print(f'DEBUG: processed_data_dir={self.processed_data_dir}')
            print(f'DEBUG: node_info.pkl exists: {osp.exists(osp.join(self.processed_data_dir, "node_info.pkl"))}')
            if self.root is None:
                raise ValueError("Cannot process raw data when root is None. Set download_processed=True or provide a valid root directory.")
            ckt_path = osp.join(self.root, 'intermediate')
            os.makedirs(ckt_path, exist_ok=True)
            print('Loading data... It might take a while')

            print('DEBUG: Loading QA data...')
            ...
            print('DEBUG: Constructing node info...')
            ...
            print('DEBUG: Creating processed_data dict...')
            processed_data = {
                'node_info': node_info,
            ...
            print(f'DEBUG: processed_data created with keys: {list(processed_data.keys())}')
            ...
            if self.processed_data_dir is not None:
                ...
                raise ValueError("Cannot save processed data when processed_data_dir is None.")
