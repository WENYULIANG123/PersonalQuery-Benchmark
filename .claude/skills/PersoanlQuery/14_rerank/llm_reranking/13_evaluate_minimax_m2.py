...
class PersonalizedMiniMaxReRanker(MiniMaxReRanker):
    ...
    def _score_by_generation(self, query: str, doc_text: str) -> float:
        ...
        selected_attrs = metadata.get('selected_attributes', [])
        
        # Build query_info for persona classification
        query_info = {
            'query': query,
            'category': category,
        ...
        # Use improved persona context builder
        from persona_utils import build_enhanced_persona_context, build_improved_prompt
        persona_context = build_enhanced_persona_context(
            category, selected_attrs, USER_ID, query_info, PROCESSING_DIR)
        
        # Build improved prompt
        prompt = build_improved_prompt(query_info, doc_text, persona_context)
