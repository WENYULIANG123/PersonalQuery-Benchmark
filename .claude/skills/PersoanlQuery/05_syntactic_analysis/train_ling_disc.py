...
用法:
    python train_ling_disc.py --disc_type t5
    python train_ling_disc.py --disc_type deberta --disc_ckpt mohdelgaar/lingconv
"""
...
class LingDiscClassifier(nn.Module):
    ...
    def __init__(self, model_name, disc_type="t5", lng_dim=40, hidden_dim=500, dropout=0.1):
        ...
        if disc_type == "t5":
            ...
            # DeBERTa 会在加载时从 disc_ckpt 获取
            raise NotImplementedError("DeBERTa type requires pretrained checkpoint")
        else:
...
def main():
    ...
    elif args.disc_type == "deberta":
        if not args.disc_ckpt:
            raise ValueError("--disc_ckpt is required for deberta type")
        model = DebertaReplacedTokenizer.from_pretrained(
            pretrained_model_name_or_path=args.disc_ckpt,
            tok_model_name=args.model_name,
            problem_type='regression',
            num_labels=args.lng_dim,
        )
        model = WrappedDeberta(model)
...
class WrappedDeberta(nn.Module):
    """包装 DeBERTa 模型以适配训练接口"""

    def __init__(self, deberta_model):
        super().__init__()
        self.model = deberta_model
        self.disc_type = "deberta"
        self.lng_dim = deberta_model.config.num_labels
