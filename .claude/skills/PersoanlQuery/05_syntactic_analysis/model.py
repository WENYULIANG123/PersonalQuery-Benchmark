...
def prepare_inputs_for_generation(
        ...
        sentence1_ling=None,
        sentence2_ling=None,
        use_original_decoder_input=True,
        **kwargs
    ):
        ...
        # Option: use only pad token for decoder input (independent generation)
        if not use_original_decoder_input:
            # Use only <pad> token - decoder generates independently
            bs = input_ids.shape[0] if input_ids.dim() > 1 else 1
            if input_ids.dim() == 1:
                input_ids = torch.full((1, 1), self.pad_token_id, dtype=torch.long, device=input_ids.device)
            else:
                input_ids = torch.full((input_ids.shape[0], 1), self.pad_token_id, dtype=torch.long, device=input_ids.device)
        ...
        elif combine_method == 'bos_replace':
            # BOS替换方法：用ling embedding替换BOS token的embedding
            # 注意：sentence2_ling在传入时可能已经是768维，不需要再次通过ling_embed
            if ling2_only:
                decoder_inputs_embeds[:, 0:1, :] = sentence2_ling
            else:
                decoder_inputs_embeds[:, 0:1, :] = sentence1_ling + sentence2_ling
            is_first_step = True  # 跳过后续的add处理
...
class EncoderDecoderVAE(LingConvT5ForConditionalGeneration):
    def __init__(self, config, args, pad_token_id, sepeos_token_id, vocab_size = 32128):
        ...
        # 语义池化：训练时关闭，推理时启用
        # 训练时decoder需要学习利用完整encoder输出
        # 推理时使用池化迫使依赖ling embedding
        self.use_semantic_pooling = False  # 默认关闭，infer时开启
        ...
        elif 'concat' in args.combine_method or 'add' in args.combine_method or 'layer_injection' in args.combine_method or 'bos_replace' in args.combine_method:
    ...
    def _process_ling_embeddings(self, sentence1_ling, sentence2_ling, 
        ...
        if sentence1_ling_embed is not None:
            ...
            if torch.isnan(sentence1_ling).any():
                print(f"DEBUG: sentence1_ling has NaN after ling_embed! input shape={sentence1_ling.shape}")
        else:
            sentence1_ling = None
        ...
        if sentence2_ling_embed is not None:
            ...
            if torch.isnan(sentence2_ling).any():
                print(f"DEBUG: sentence2_ling has NaN after ling_embed! input shape={sentence2_ling.shape}, max={sentence2_ling.max():.4f}, min={sentence2_ling.min():.4f}")
        else:
            sentence2_ling = None
    ...
    def decode(self,
              ...
              return_logits=False,
              use_semantic_pooling=None,  # 新增: 是否使用语义池化
              **kwargs
              ):

        # 如果use_semantic_pooling未指定，使用模型默认值
        if use_semantic_pooling is None:
            use_semantic_pooling = self.use_semantic_pooling

        # 语义池化: 将encoder输出压缩成单个向量，迫使decoder依赖ling embedding
        # 必须先做池化，再计算bs
        if use_semantic_pooling and encoder_outputs is not None:
            enc_hidden = encoder_outputs[0]  # [batch, seq_len, hidden]
            bs = enc_hidden.shape[0]
            # Mean pooling: [batch, seq_len, hidden] -> [batch, 1, hidden]
            pooled_enc = enc_hidden.mean(dim=1, keepdim=True)
            # 创建一个符合格式的encoder_outputs，支持tuple风格的索引
            class PooledEncoderOutput:
                ...
                def __len__(self):
                    return 1
            encoder_outputs_pooled = PooledEncoderOutput(pooled_enc)
            # 创建一个全1的attention mask（因为只有一个位置）
            encoder_attention_mask_pooled = torch.ones(bs, 1, device=enc_hidden.device)
            cache = {'original_encoder_outputs': encoder_outputs, 'original_encoder_attention_mask': encoder_attention_mask, 'use_semantic_pooling': True}
        else:
            bs = encoder_outputs[0].shape[0]
            encoder_outputs_pooled = encoder_outputs
            encoder_attention_mask_pooled = encoder_attention_mask
            cache = {}

        if decoder_inputs_embeds is None:
            if self.args.combine_method in ('embed_concat', 'decoder_concat', 'decoder_add',
                                        'logits_add', 'decoder_add_first', 'layer_injection', 'bos_replace'):
                sentence1_ling, sentence2_ling, cache = self._process_ling_embeddings(
                    sentence1_ling, sentence2_ling,
                    sentence1_ling_embed, sentence2_ling_embed, bs
                )
        ...
        if generate:
            ...
            # If we need logits, use manual greedy decoding instead of standard generate()
            if return_logits:
                dec_output, logits_seq = self._greedy_decode_with_logits(
                    encoder_outputs=encoder_outputs_pooled,
                    encoder_attention_mask=encoder_attention_mask_pooled,
                    sentence1_ling=sentence1_ling,
                    sentence2_ling=sentence2_ling,
                    logits_processor=logits_processor,
                )
                cache['logits'] = logits_seq
                ...
                dec_output = generate_fn(
                        attention_mask=encoder_attention_mask_pooled,
                        encoder_outputs=encoder_outputs_pooled,
                        sentence1_ling=sentence1_ling,
                        sentence2_ling=sentence2_ling,
                        logits_processor = logits_processor,
        ...
        if decoder_inputs_embeds is None:
            ...
            if self.args.combine_method == 'decoder_concat':
                ...
                if self.args.ling2_only:
                    ...
                    decoder_inputs_embeds = decoder_inputs_embeds + sentence1_ling + sentence2_ling
            elif self.args.combine_method == 'bos_replace':
                # BOS替换方法：用ling embedding替换BOS token的embedding
                # 注意：sentence2_ling已经在_process_ling_embeddings中转换到768D了
                # 直接使用已转换的embedding，不需要再次调用ling_embed
                if self.args.ling2_only:
                    decoder_inputs_embeds[:, 0:1, :] = sentence2_ling
                else:
                    decoder_inputs_embeds[:, 0:1, :] = sentence1_ling + sentence2_ling
                # 调试：检查是否有NaN
                if torch.isnan(decoder_inputs_embeds).any():
                    print(f"WARNING: NaN in decoder_inputs_embeds after bos_replace! sentence2_ling has NaN: {torch.isnan(sentence2_ling).any()}")
    ...
    def _greedy_decode_with_logits(self, encoder_outputs, encoder_attention_mask,
                                   sentence1_ling, sentence2_ling, logits_processor):
        """
        Manual greedy decoding that returns logits at each step.
        This is needed because HuggingFace generate() only returns token IDs, not logits.
        """
        bs = encoder_outputs[0].shape[0]
        ...
        # Prepare ling embeddings - these will be added at first step
        if sentence1_ling is not None and sentence2_ling is not None:
            ling_to_add = sentence1_ling + sentence2_ling
        elif sentence2_ling is not None:
            ling_to_add = sentence2_ling
        else:
            ling_to_add = None

        # Initialize decoder input with pad token
        decoder_input_ids = torch.full((bs, 1), self.pad_token_id, dtype=torch.long, device=device)

        # Store logits and generated tokens at each step
        logits_seq = []
        generated_tokens = [decoder_input_ids]

        max_length = self.args.max_length

        # Track past key values for efficient decoding
        past_key_values = None
        ...
        for step in range(max_length):
            ...
            # Add ling embeddings at first step (for decoder_add and similar methods)
            if step == 0 and ling_to_add is not None:
                if self.args.combine_method in ('decoder_add', 'decoder_add_first', 'layer_injection'):
                    if self.args.ling2_only:
                        decoder_input_embeds = decoder_input_embeds + sentence2_ling
                    else:
                        decoder_input_embeds = decoder_input_embeds + sentence1_ling + sentence2_ling
            ...
            # Only add encoder attention mask if it's meaningful (not all ones)
            if encoder_attention_mask is not None and encoder_attention_mask.sum() != encoder_attention_mask.numel():
            ...
            logits = self.lm_head(decoder_outputs.last_hidden_state)  # [bs, seq_len, vocab_size]
            logits = logits[:, -1, :]  # [bs, vocab_size]

            # Apply logits processor (e.g., LogitsAdd for ling embedding bias)
            if logits_processor:
                logits = logits_processor(decoder_input_ids, logits)

            logits_seq.append(logits)

            # Sampling decoding with temperature + nucleus sampling
            ...
            if temperature > 0:
                ...
                # Nucleus sampling (top_p filtering)
                ...
                # Create a tensor to scatter the filtered probabilities back
                # First, unsort: scatter filtered values back to original positions
                ...
                next_token_id = torch.multinomial(filtered_probs, num_samples=1)  # [bs, 1]
            else:
                next_token_id = logits.argmax(dim=-1, keepdim=True)  # [bs, 1]
        ...
        # Stack logits: [seq_len, bs, vocab_size] -> [bs, seq_len, vocab_size]
        logits_seq = torch.stack(logits_seq, dim=1)  # [bs, seq_len, vocab_size]

        # Concatenate all generated tokens to get full sequence
        # Skip the first pad token: generated_tokens[0] is pad, then we have seq_len tokens
        generated_ids = torch.cat(generated_tokens[1:], dim=1)  # [bs, seq_len]

        return generated_ids, logits_seq
    ...
    def forward(self,
        ...
        cache = {}
        # 调试：检查输入是否有NaN
        if torch.isnan(sentence2_ling).any() if sentence2_ling is not None else False:
            print(f"DEBUG: sentence2_ling has NaN! shape={sentence2_ling.shape if sentence2_ling is not None else None}")
    ...
    def infer_with_cache(self, batch):
        ...
        dec_output, cache2 = self.decode(
            ...
            use_semantic_pooling=True,  # 推理时启用语义池化
        )
        cache.update(cache2)
        return dec_output, cache

    def infer_with_cache_and_logits(self, batch, return_logits=False):
        """
        Similar to infer_with_cache but can return logits for QC inference.
        When return_logits=True, uses manual greedy decoding to get logits at each step.
        """
        batch = self._normalize_batch_dict(batch)
        encoder_outputs, encoder_attention_mask, cache = self.encode(
            input_ids=batch.get("input_ids"),
        ...
        if return_logits:
            # Use manual greedy decoding to get logits for each step
            dec_output, logits_cache = self.decode(
                sentence2_input_ids=batch.get("labels"),
                sentence1_ling=batch.get("sentence1_ling"),
                sentence2_ling=batch.get("sentence2_ling"),
                encoder_outputs=encoder_outputs,
            ...
            cache.update(logits_cache)
            return dec_output, cache
        else:
            # Use generate=False to get logits via teacher forcing
            dec_output, cache2 = self.decode(
                sentence2_input_ids=batch.get("labels"),
                sentence1_ling=batch.get("sentence1_ling"),
                sentence2_ling=batch.get("sentence2_ling"),
            ...
            cache.update(cache2)
            # Also return logits from dec_output for convenience
            cache['logits'] = dec_output.logits
            return dec_output, cache
    ...
    def infer_with_feedback_BP(self, ling_disc, sem_emb, batch, tokenizer):
        ...
        def line_search():
            ...
            while patience > 0:
                ...
                # Compute semantic probability if sem_emb has compare_sem method
                if hasattr(sem_emb, 'compare_sem'):
                    sem_prob = torch.sigmoid(sem_emb.compare_sem(**batch)).item()
                else:
                    # For shared encoder, use cosine similarity as fallback
                    with torch.no_grad():
                        enc_out = sem_emb(
                            input_ids=batch.get('sentence1_input_ids'),
                            attention_mask=batch.get('sentence1_attention_mask')
                        )
                        s1_emb = enc_out.last_hidden_state.mean(1)

                        enc_out2 = sem_emb(
                            input_ids=batch.get('sentence2_input_ids'),
                            attention_mask=batch.get('sentence2_attention_mask')
                        )
                        s2_emb = enc_out2.last_hidden_state.mean(1)

                        cos_sim = F.cosine_similarity(s1_emb, s2_emb, dim=-1)
                        sem_prob = cos_sim.item() if cos_sim.dim() == 0 else cos_sim[0].item()
        ...
        def get_loss(param):
            ...
            if self.args.feedback_param == 'logits':
                ...
                # Use infer_with_cache_and_logits with return_logits=True for self-regressive decoding
                pred, cache = self.infer_with_cache_and_logits(batch, return_logits=True)
                logits = cache.get('logits')
                if logits is None:
                    # Fallback: try to get from forward pass output
                    logits = pred.logits if hasattr(pred, 'logits') else None
                    if logits is not None:
                        # For teacher forcing, logits are [bs, seq_len, vocab_size]
                        # We only care about the last position for prediction
                        logits = logits[:, -1:, :]  # [bs, 1, vocab_size]
        ...
        if self.args.feedback_param == 'l':
            ...
            # Use return_logits=True to get logits from self-regressive decoding
            _, cache = self.infer_with_cache_and_logits(batch, return_logits=True)
            logits = cache.get('logits')
            if logits is None:
                # Fallback: try teacher forcing logits
                _, cache = self.infer_with_cache_and_logits(batch, return_logits=False)
                logits = cache.get('logits')
                if logits is None:
                    raise RuntimeError("Failed to get logits for QC inference")
                # For teacher forcing, take last position
                logits = logits[:, -1:, :]
            param = torch.nn.Parameter(logits, requires_grad = True)
        target_np = batch['sentence2_ling'][0].cpu().numpy()
        while True:
            loss, pred = get_loss(param)
            pred_text = tokenizer.batch_decode(pred.cpu().numpy() if isinstance(pred, torch.Tensor) else pred,
