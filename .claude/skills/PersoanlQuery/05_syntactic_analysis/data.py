...
def load_data(args, tokenizer, return_data=False):
    dataset_path = os.path.join(args.data_dir, args.data)
    if os.path.exists(dataset_path):
        data = load_from_disk(dataset_path)
    else:
        # Download from HuggingFace if local path doesn't exist
        from datasets import load_dataset
        print(f"Downloading dataset from HuggingFace: mohdelgaar/{args.data}")
        data = load_dataset(f"mohdelgaar/{args.data}")
