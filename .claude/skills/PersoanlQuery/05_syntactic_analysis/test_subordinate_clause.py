...
def main():
    ...
    print("\n【细粒度均匀值测试】:")
    for val in [0.50, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]:
        ...
        try:
            ...
            results.append((f"均匀{val:.2f}", word_count, clause_count, pred_text))
    ...
    # 总结
    print("\n" + "=" * 70)
    print("结果汇总（按从句数量降序，词数升序）:")
    print("=" * 70)
    results.sort(key=lambda x: (-x[2], x[1]))
    for name, word_count, clause_count, text in results:
        print(f"{name}: {clause_count}从句, {word_count}词 - {text}")
