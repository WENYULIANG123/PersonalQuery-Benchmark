...
class SpellingDifficultyScorer:
    ...
    def predict_difficulty_by_error_type(self, word, user_profile=None):
        """
        Predict difficulty for each of the 9 error types separately.

        For each error type, creates a virtual user profile with that error type = 1.0
        and all others = 0, then multiplies by the user's actual error frequency.

        Returns: dict mapping error_type -> (difficulty_score, weighted_score)
        """
        word_clean = re.sub(r'[^a-zA-Z]', '', word).lower()
        if len(word_clean) < 3:
            return {}
        ...
        # Error type mapping
        error_types = ['Hard Word', 'Deletion', 'Homophone', 'Suffix', 'Extra Space', 'Transposition', 'Insertion', 'Substitution', 'Scramble']

        # Get user's actual error frequencies
        user_frequencies = [0.0] * 9
        if user_profile and 'spelling' in user_profile:
            spelling_stats = user_profile['spelling']
            spelling_total = user_profile.get('spelling_total', sum(spelling_stats.values()))
            if spelling_total > 0:
                for i, error_type in enumerate(error_types):
                    user_frequencies[i] = spelling_stats.get(error_type, 0.0) / spelling_total

        results = {}

        with torch.no_grad():
            for i, error_type in enumerate(error_types):
                # Create virtual user profile: this error type = 1.0, others = 0
                virtual_user_features = [0.0] * 9
                virtual_user_features[i] = 1.0

                X_user = torch.tensor([virtual_user_features], dtype=torch.float32).to(self.device)
                score = self.model(X_chars, X_feats, X_user)

                # Multiply by user's actual frequency for this error type
                user_frequency = user_frequencies[i] if user_frequencies[i] > 0 else 0.01  # Avoid zero
                weighted_score = score.item() * user_frequency

                results[error_type] = {
                    'difficulty_score': score.item(),
        ...
        for word in words:
            ...
            if len(clean_word) > 3:
                ...
                if diff_score > threshold:
                    # Return the ORIGINAL word (not cleaned) so replacement works correctly
                    vulnerable.append((word, diff_score))
