#!/usr/bin/env python3
"""Qwen2-7B Reranker Evaluation (Standard + Personalized) - Using cached E5 candidates"""
...
MAX_CONCURRENT = 3
SIM_WEIGHT = 0.1
FIRST_STAGE_MODEL = "E5"


class SiliconFlowClient:
    """SiliconFlow API client for Qwen"""
    def __init__(self, model: str = "Qwen/Qwen2-7B-Instruct"):
        self.api_key = "sk-drezmfyckjkmxixpiblvbwdhypjbrsoyvmeertajtupiqnnj"
        self.base_url = "https://api.siliconflow.cn/v1"
        self.model = model

    def call(self, prompt: str, max_tokens: int = 10, temperature: float = None, max_retries: int = 5) -> str:
        headers = {
...
class QwenReRanker:
    def __init__(self, base_retriever, top_k: int = 50, model: str = "Qwen/Qwen2-7B-Instruct"):
        self.base_retriever = base_retriever
        self.top_k = top_k
        self.model_name = model
        self.llm_client = SiliconFlowClient(model=model)
        self.documents = None
...
class PersonalizedQwenReRanker(QwenReRanker):
    def __init__(self, base_retriever, top_k: int = 50, persona_dir: str = None, processing_dir: str = None, model: str = "Qwen/Qwen2-7B-Instruct"):
...
def main():
    ...
    log_with_timestamp(f"Qwen2-7B Reranker Evaluation (Standard + Personalized)")
    ...
    common_metadata = {'user_id': USER_ID, 'timestamp': datetime.now().isoformat(),
        ...
        'k_values': K_VALUES, 'model': MODEL_NAME, 'first_stage': FIRST_STAGE_MODEL}
    ...
    log_with_timestamp("=" * 50)
    log_with_timestamp(f"=== Standard ({FIRST_STAGE_MODEL} -> Qwen2-7B) ===")

    first_stage_std = CachedE5(e5_target_cache)
    first_stage_std.fit(documents, all_metadata)
    qwen_std = QwenReRanker(first_stage_std, top_k=TOP_K_CANDIDATES, model=MODEL_NAME)
    qwen_std.fit(documents, all_metadata)

    target_std = evaluate_retriever(qwen_std, target_queries, list(all_asins), K_VALUES)

    for metrics, qtype, suffix in [(target_std, 'target_user', 'target')]:
        out = {**common_metadata, 'query_type': qtype, 'retriever': 'qwen_reranker', 'personalized': False, 'metrics': metrics}
        with open(os.path.join(OUTPUT_DIR, f"retrieval_qwen_{FIRST_STAGE_MODEL}_{suffix}_{USER_ID}.json"), 'w') as f:
            json.dump(out, f, indent=2)
    log_with_timestamp(f"Standard Target: {target_std}")

    log_with_timestamp("=" * 50)
    log_with_timestamp(f"=== Personalized ({FIRST_STAGE_MODEL} -> Qwen2-7B + Persona) ===")

    first_stage_per = CachedE5(e5_target_cache)
    first_stage_per.fit(documents, all_metadata)
    qwen_per = PersonalizedQwenReRanker(first_stage_per, top_k=TOP_K_CANDIDATES, 
                                       persona_dir=PERSONA_DIR, processing_dir=PROCESSING_DIR, 
                                       model=MODEL_NAME)
    qwen_per.fit(documents, all_metadata, queries=target_queries, user_id=USER_ID)

    target_per = evaluate_retriever(qwen_per, target_queries, list(all_asins), K_VALUES)

    for metrics, qtype, suffix in [(target_per, 'target_user', 'personalized_target')]:
        out = {**common_metadata, 'query_type': qtype, 'retriever': 'qwen_reranker', 'personalized': True, 'metrics': metrics}
        with open(os.path.join(OUTPUT_DIR, f"retrieval_qwen_{FIRST_STAGE_MODEL}_{suffix}_{USER_ID}.json"), 'w') as f:
            json.dump(out, f, indent=2)
