...
def generate_personalized_query(asin, category, product_attrs, public_attrs=None):
    ...
    prompt = f"""Task: Organize the given attributes into a natural search query.
Category: {category}
Attributes: {attrs_str}

STRICT CONSTRAINTS:
1. USE EXACT ATTRIBUTE NAMES: Keep each attribute name exactly as given. Do NOT rephrase or rewrite.
2. ORGANIZE INTO A QUERY: Combine the attributes into a coherent search query using natural connecting words.
3. ADD MINIMAL CONTEXT: Only add simple words like "I need", "with", "and", "for my projects" to make it a sentence.
4. LENGTH: MUST be exactly 20-30 words. No exceptions.
5. FORMAT: Output ONLY the query text, no prefixes or explanations.

Example (attributes "Acid-free", "Archival Quality"):
"I need acid-free materials with archival quality that will preserve my projects for a long time."

Example (attributes "Precision", "Stitch Control", "Durability"):
"I'm looking for a sewing machine with precision, stitch control, and durability for my sewing projects."
...
STRICT CONSTRAINTS:
1. USE EXACT ATTRIBUTE NAMES: Keep each attribute name exactly as given.
2. LENGTH: MUST be exactly 20-30 words. Your previous output had {word_count} words, which is TOO SHORT. Add more connecting words.
3. FORMAT: Output ONLY the query text.
