#!/usr/bin/env python3
"""
Stage 13: Dense (ANCE/MiniLM) Retrieval Evaluation (Standalone)

Dense retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_dense_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("Dense (ANCE/MiniLM) Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    output_data = {
        **common_metadata,
        ...
        'metrics': dense_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_dense_target_{USER_ID}.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_dense_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (Dense/ANCE):")
    for k, v in dense_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (Dense/ANCE):")
    for k, v in dense_mass.items():
        log_with_timestamp(f"  {k}: {v}")
