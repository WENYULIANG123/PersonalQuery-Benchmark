#!/usr/bin/env python3
"""
Stage 13: E5-large-v2 Retrieval Evaluation (Standalone)

E5 retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_e5_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("E5-large-v2 Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    common_metadata = {
        ...
        'k_values': K_VALUES,
        'retriever': 'e5_large_v2'
    }

    log_with_timestamp("=" * 50)
    log_with_timestamp("Evaluating target user queries...")
    e5_candidates_file = os.path.join(OUTPUT_DIR, f"e5_candidates_{USER_ID}_target.json")
    e5_target = evaluate_retriever(e5_retriever, target_queries, list(all_asins), K_VALUES, save_candidates_path=e5_candidates_file)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_e5_target_{USER_ID}.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_e5_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (E5-large-v2):")
    for k, v in e5_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (E5-large-v2):")
    for k, v in e5_mass.items():
        log_with_timestamp(f"  {k}: {v}")
