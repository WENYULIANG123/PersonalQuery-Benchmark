...
def main():
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_colbert_target_{USER_ID}.json")
    ...
    output_data = {
        **common_metadata,
        ...
        'metrics': colbert_mass
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_colbert_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (ColBERTv2):")
    for k, v in colbert_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (ColBERTv2):")
    for k, v in colbert_mass.items():
        log_with_timestamp(f"  {k}: {v}")
