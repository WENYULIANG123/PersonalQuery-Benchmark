...
def main():
    ...
    bert_target = evaluate_retriever(bert_reranker, target_queries, list(all_asins), K_VALUES)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_bert_reranker_target_{USER_ID}.json")
    ...
    bert_mass = evaluate_retriever(bert_reranker, mass_queries, list(all_asins), K_VALUES)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_bert_reranker_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (BERT Reranker):")
