#!/usr/bin/env python3
"""GLM-5 Reranker Evaluation (Standard + Personalized) - Using cached E5 candidates"""
...
class GLMReRanker:
    def __init__(self, base_retriever, top_k: int = 50, model: str = "GLM-5"):
...
Relevance Score:'''
    ...
    def search(self, query: str, top_k: int = 10) -> List[Tuple[str, float]]:
        ...
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT) as executor:
            futures = {executor.submit(score_doc, asin, idx): asin for idx, (asin, _) in enumerate(candidates)}
...
class PersonalizedGLMReRanker(GLMReRanker):
    def __init__(self, base_retriever, top_k: int = 50, persona_dir: str = None, model: str = "GLM-5"):
    ...
    def _build_persona_context(self, category: str, selected_attributes: List[Dict]) -> str:
        persona = self._load_persona(category)
        if not persona:
            return ""
        dimension_personas = persona.get('dimension_personas', {})
        contexts = [f"  - {dim}: {dimension_personas[dim]}" for attr in selected_attributes
            if (dim := attr.get('dimension', '')) in dimension_personas]
...
def main():
    ...
    log_with_timestamp(f"GLM-5 Reranker Evaluation (Standard + Personalized)")
    ...
    # e5_mass = CachedE5(e5_mass_cache)
    # e5_mass.fit(documents, all_metadata)
    # glm_std_mass = GLMReRanker(e5_mass, top_k=TOP_K_CANDIDATES, model=GLM_MODEL)
    ...
    glm_per = PersonalizedGLMReRanker(e5_per, top_k=TOP_K_CANDIDATES, persona_dir=PERSONA_DIR, model=GLM_MODEL)
    ...
    # Skip mass query reranking - only evaluate target queries
    # e5_per_mass = CachedE5(e5_mass_cache)
    # e5_per_mass.fit(documents, all_metadata)
    # glm_per_mass = PersonalizedGLMReRanker(e5_per_mass, top_k=TOP_K_CANDIDATES, persona_dir=PERSONA_DIR, model=GLM_MODEL)
