#!/usr/bin/env python3
"""
Stage 13: BGE-large-en Retrieval Evaluation (Standalone)

BGE retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_bge_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("BGE-large-en Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_bge_target_{USER_ID}.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_bge_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (BGE-large-en):")
    for k, v in bge_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (BGE-large-en):")
    for k, v in bge_mass.items():
        log_with_timestamp(f"  {k}: {v}")
