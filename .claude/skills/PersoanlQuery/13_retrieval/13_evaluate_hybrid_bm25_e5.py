#!/usr/bin/env python3
"""
Stage 13: Hybrid (BM25 + E5) Retrieval Evaluation (Standalone)

Hybrid BM25+E5 retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_hybrid_bm25_e5_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("Hybrid (BM25 + E5) Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building Hybrid retriever (BM25 + E5)...")
    bm25 = BM25()
    ...
    hybrid_retriever = HybridRetriever([bm25, e5_retriever], k=RRF_K)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_hybrid_bm25_e5_target_{USER_ID}.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_hybrid_bm25_e5_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (Hybrid BM25+E5):")
    for k, v in hybrid_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (Hybrid BM25+E5):")
