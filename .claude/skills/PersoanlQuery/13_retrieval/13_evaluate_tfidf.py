#!/usr/bin/env python3
"""
Stage 13: TF-IDF Retrieval Evaluation (Standalone)

TF-IDF retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_tfidf_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("TF-IDF Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building TF-IDF retriever...")
    
    # Build TF-IDF index
    log_with_timestamp("=" * 50)
    log_with_timestamp("Building TF-IDF...")
    ...
    output_data = {
        **common_metadata,
        ...
        'metrics': tfidf_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_tfidf_target_{USER_ID}.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_tfidf_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (TF-IDF):")
    for k, v in tfidf_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (TF-IDF):")
    for k, v in tfidf_mass.items():
        log_with_timestamp(f"  {k}: {v}")
