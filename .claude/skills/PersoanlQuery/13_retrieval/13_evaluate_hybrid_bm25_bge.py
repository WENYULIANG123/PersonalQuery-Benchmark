#!/usr/bin/env python3
"""
Stage 13: Hybrid (BM25 + BGE) Retrieval Evaluation (Standalone)

Hybrid BM25+BGE retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_hybrid_bm25_bge_{user_id}.json
"""
...
def main():
    ...
    log_with_timestamp("Hybrid (BM25 + BGE) Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp(f"RRF k: {RRF_K}")
    ...
    log_with_timestamp("Building Hybrid retriever (BM25 + BGE)...")
    bm25 = BM25()
    ...
    hybrid_retriever = HybridRetriever([bm25, bge_retriever], k=RRF_K)
    hybrid_retriever.fit(documents, all_metadata)
    ...
    common_metadata = {
        ...
        'k_values': K_VALUES,
        'retriever': 'hybrid_bm25_bge',
        'rrf_k': RRF_K
    }

    log_with_timestamp("=" * 50)
    log_with_timestamp("Evaluating target user queries...")
    hybrid_target = evaluate_retriever(hybrid_retriever, target_queries, list(all_asins), K_VALUES)
    
    output_data = {
        **common_metadata,
        'query_type': 'target_user',
        'metrics': hybrid_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_hybrid_bm25_bge_target_{USER_ID}.json")
    ...
    hybrid_mass = evaluate_retriever(hybrid_retriever, mass_queries, list(all_asins), K_VALUES)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_hybrid_bm25_bge_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (Hybrid BM25+BGE):")
    for k, v in hybrid_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (Hybrid BM25+BGE):")
    for k, v in hybrid_mass.items():
        log_with_timestamp(f"  {k}: {v}")
