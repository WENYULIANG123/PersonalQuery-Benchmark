#!/usr/bin/env python3
"""
Stage 13: GritLM Retrieval Evaluation (Standalone)

GritLM uses Muennighoff/SGPT-7B-weightedmean-nli-bitfit embedding model.
This is the same model used in STaRK for VSS retrieval.
...
Output: result/personal_query/13_retrieval/retrieval_gritlm_{user_id}.json
"""
...
GritLMRetriever = retrievers.GritLMRetriever
...
def main():
    ...
    log_with_timestamp("GritLM Retrieval Evaluation (Standalone)")
    log_with_timestamp("Model: Muennighoff/SGPT-5.8B-weightedmean-nli-bitfit")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building GritLM...")
    gritlm_retriever = GritLMRetriever()
    gritlm_retriever.fit(documents, all_metadata)
    ...
    common_metadata = {
        ...
        'k_values': K_VALUES,
        'retriever': 'gritlm_5.8b',
        'model': 'Muennighoff/SGPT-5.8B-weightedmean-nli-bitfit'
    }

    log_with_timestamp("=" * 50)
    log_with_timestamp("Evaluating target user queries...")
    gritlm_target = evaluate_retriever(gritlm_retriever, target_queries, list(all_asins), K_VALUES)
    
    output_data = {
        **common_metadata,
        'query_type': 'target_user',
        'metrics': gritlm_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_gritlm_target_{USER_ID}.json")
    ...
    gritlm_mass = evaluate_retriever(gritlm_retriever, mass_queries, list(all_asins), K_VALUES)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_gritlm_mass_{USER_ID}.json")
    with open(output_file, 'w') as f:
        json.dump(output_data, f, indent=2)
    log_with_timestamp(f"Saved: {output_file}")
    ...
    log_with_timestamp("\nTarget User Queries (GritLM):")
    for k, v in gritlm_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (GritLM):")
    for k, v in gritlm_mass.items():
        log_with_timestamp(f"  {k}: {v}")
