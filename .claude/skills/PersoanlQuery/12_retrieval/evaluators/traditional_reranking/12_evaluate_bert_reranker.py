#!/usr/bin/env python3
"""
Stage 13: BERT Reranker (BM25 + BERT cross-encoder) Retrieval Evaluation (Standalone)

BERT Reranker retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_bert_reranker_{user_id}.json
"""
...
DEFAULT_META_FILE = os.path.join(DEFAULT_BASE_DIR, "data/Amazon-Reviews-2018/raw/meta_Arts_Crafts_and_Sewing.json")
...
DEFAULT_BERT_TOP_K = 50


def main():
    parser = argparse.ArgumentParser(description="BERT Reranker Retrieval Evaluation")
    ...
    BERT_TOP_K = DEFAULT_BERT_TOP_K

    log_with_timestamp("=" * 60)
    log_with_timestamp("BERT Reranker Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp(f"BERT top_k: {BERT_TOP_K}")
    ...
    log_with_timestamp("Building BM25 + BERT reranker...")
    bm25_for_bert = BM25()
    bert_reranker = BERTReRanker(bm25_for_bert, top_k=BERT_TOP_K)
    ...
    bert_candidates_file = os.path.join(OUTPUT_DIR, f"bert_reranker_candidates_{QUERY_MODE}_{USER_ID}_target.json")
    bert_target = evaluate_retriever(bert_reranker, target_queries, list(all_asins), K_VALUES, save_candidates_path=bert_candidates_file)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_bert_reranker_{QUERY_MODE}_{USER_ID}.json")
    ...
    log_with_timestamp(f"\nTarget User Queries (BERT Reranker, {QUERY_MODE} mode):")
    for k, v in bert_target.items():
        log_with_timestamp(f"  {k}: {v}")
    
    log_with_timestamp("\nMass Market Queries (BERT Reranker):")
    for k, v in bert_mass.items():
        log_with_timestamp(f"  {k}: {v}")
