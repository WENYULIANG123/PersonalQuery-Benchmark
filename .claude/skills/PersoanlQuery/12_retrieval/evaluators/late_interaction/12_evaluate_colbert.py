#!/usr/bin/env python3
"""
Stage 13: ColBERTv2 Retrieval Evaluation (Standalone)

ColBERT retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_colbert_{user_id}.json
"""
...
def main():
    parser = argparse.ArgumentParser(description="ColBERT Retrieval Evaluation")
    ...
    log_with_timestamp("ColBERTv2 Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building ColBERTv2...")
    ...
    colbert_candidates_file = os.path.join(OUTPUT_DIR, f"colbert_candidates_{QUERY_MODE}_{USER_ID}_target.json")
    colbert_target = evaluate_retriever(colbert, target_queries, list(all_asins), K_VALUES, save_candidates_path=colbert_candidates_file)
    
    output_data = {
        **common_metadata,
        'query_type': 'target_user',
        'metrics': colbert_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_colbert_{QUERY_MODE}_{USER_ID}.json")
    ...
    log_with_timestamp(f"\nTarget User Queries (ColBERTv2, {QUERY_MODE} mode):")
