...
class PersonalizedMiniMaxReRanker(MiniMaxReRanker):
    ...
    def _build_persona_context(self, category: str, selected_attributes: List[Dict]) -> str:
        if not selected_attributes:
            log_with_timestamp(f"⚠️ No selected_attributes provided for category: {category}")
            return ""

        log_with_timestamp(f"🔧 Building persona context for category: {category}, selected_attrs: {len(selected_attributes)}")

        selected_dims = set(attr.get('dimension', '') for attr in selected_attributes if attr.get('dimension'))
        log_with_timestamp(f"  Selected dimensions: {selected_dims}")

        all_attrs = self._load_processing_attrs(category)
        log_with_timestamp(f"  Loaded {len(all_attrs)} persona attributes")

        if not all_attrs:
            log_with_timestamp(f"  ⚠️ No persona attributes found, using fallback with selected_attributes values")
            ...
            log_with_timestamp(f"  Fallback context length: {len(result)}")
            return result
        ...
        log_with_timestamp(f"  Matched dimensions in persona: {list(attrs_by_dim.keys())}")
        ...
        result = "User Preferences:\n" + "\n".join(contexts) if contexts else ""
        log_with_timestamp(f"  ✅ Final persona context length: {len(result)}, lines: {len(contexts)}")
        return result
    ...
    def _score_by_generation(self, query: str, doc_text: str) -> float:
        ...
        # Debug: log if persona context is being used
        if persona_context:
            log_with_timestamp(f"🔍 Using personalized context for query (length: {len(persona_context)})")
