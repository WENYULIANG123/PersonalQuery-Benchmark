#!/usr/bin/env python3
"""
Stage 13: ANCE Retrieval Evaluation (Standalone)

ANCE retrieval evaluation without command-line parameters.
...
Output: result/personal_query/13_retrieval/retrieval_ance_{user_id}.json
"""
...
ANCERetriever = retrievers.ANCERetriever
...
def main():
    parser = argparse.ArgumentParser(description="ANCE Retrieval Evaluation")
    ...
    log_with_timestamp("ANCE Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building ANCE retriever...")
    ance_retriever = ANCERetriever()
    ance_retriever.fit(documents, all_metadata)
    ...
    ance_candidates_file = os.path.join(OUTPUT_DIR, f"ance_candidates_{QUERY_MODE}_{USER_ID}_target.json")
    ance_target = evaluate_retriever(ance_retriever, target_queries, list(all_asins), K_VALUES, save_candidates_path=ance_candidates_file)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_ance_{QUERY_MODE}_{USER_ID}.json")
    ...
    log_with_timestamp(f"\nTarget User Queries (ANCE, {QUERY_MODE} mode):")
    for k, v in ance_target.items():
