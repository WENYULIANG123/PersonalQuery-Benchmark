...
MPNetRetriever = retrievers.MPNetRetriever
...
def main():
    parser = argparse.ArgumentParser(description="MPNet Retrieval Evaluation")
    ...
    log_with_timestamp("MPNet Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building MPNet retriever...")
    mpnet_retriever = MPNetRetriever()
    mpnet_retriever.fit(documents, all_metadata)
    ...
    mpnet_target = evaluate_retriever(mpnet_retriever, target_queries, list(all_asins), K_VALUES)
    
    output_data = {
        **common_metadata,
        'query_type': 'target_user',
        'metrics': mpnet_target
    }
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_mpnet_{QUERY_MODE}_{USER_ID}.json")
    ...
    log_with_timestamp(f"\nTarget User Queries (MPNet, {QUERY_MODE} mode):")
    for k, v in mpnet_target.items():
