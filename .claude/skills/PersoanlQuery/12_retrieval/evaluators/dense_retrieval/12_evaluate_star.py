...
STARRetriever = retrievers.STARRetriever
...
def main():
    parser = argparse.ArgumentParser(description="STAR Retrieval Evaluation")
    ...
    log_with_timestamp("STAR Retrieval Evaluation (Standalone)")
    log_with_timestamp("=" * 60)
    ...
    log_with_timestamp("Building STAR retriever...")
    star_retriever = STARRetriever()
    star_retriever.fit(documents, all_metadata)
    ...
    star_candidates_file = os.path.join(OUTPUT_DIR, f"star_candidates_{QUERY_MODE}_{USER_ID}_target.json")
    star_target = evaluate_retriever(star_retriever, target_queries, list(all_asins), K_VALUES, save_candidates_path=star_candidates_file)
    ...
    output_file = os.path.join(OUTPUT_DIR, f"retrieval_star_{QUERY_MODE}_{USER_ID}.json")
    ...
    log_with_timestamp(f"\nTarget User Queries (STAR, {QUERY_MODE} mode):")
    for k, v in star_target.items():
