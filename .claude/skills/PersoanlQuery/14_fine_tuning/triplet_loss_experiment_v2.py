...
def train_model_simple(
    ...
    loss_func = TripletLoss(model=model, triplet_margin=0.5)
    ...
    for epoch in range(config['epochs']):
        ...
        for batch in progress_bar:
            ...
            inputs = model.tokenize(batch.texts)
            for key in inputs:
                inputs[key] = inputs[key].to(device)
            
            embeddings_list = []
            for text_group in batch.texts:
                if isinstance(text_group, list):
                    emb = [model.encode(t, convert_to_tensor=True) for t in text_group]
                    embeddings_list.append(torch.stack(emb))
                else:
                    emb = model.encode(text_group, convert_to_tensor=True)
                    embeddings_list.append(emb)
            
            loss = loss_func(embeddings_list[0], embeddings_list[1], embeddings_list[2])
