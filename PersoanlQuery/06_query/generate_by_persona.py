...
# 特征提取
# ========================================
def extract_relcl_features(text: str) -> dict:
    """提取relcl特征"""
    doc = nlp_trf(text)
    tokens = [t for t in doc if not t.is_punct and not t.is_space]
    num_tokens = len(tokens)
    if num_tokens == 0:
        return {'relcl': 0.0, 'relcl_count': 0, 'word_count': 0}
    relcl_count = sum(1 for t in tokens if t.dep_ == 'relcl')
    return {
        'relcl': relcl_count / num_tokens,
...
def build_persona_prompt(persona: dict, product_attrs: dict) -> str:
    ...
    if length_label == 'short':
        target_length = min(avg_length, 15)
    elif length_label == 'long':
        target_length = max(avg_length, 30)
    else:
...
- Use ONLY simple descriptive phrases joined by commas or 'and'.
- Example: "Dritz curved seam ripper priced at $2.99, perfect for quilting and alterations"
- Example: "Dritz curved seam ripper, plastic handle, $2.99, ideal for sewing projects"
- BAD example (DO NOT DO THIS): "Dritz seam ripper that is sharp..." - Contains 'that'!"""
        else:
            # 使用 relcl_type_distribution 格式，但只取 target_relcl_count 个
            all_clauses = []
...
def parse_query(text_content: str) -> dict:
    ...
    try:
        ...
        if query:
            feats = extract_relcl_features(query)
            return {
                'query': query,
                'word_count': feats['word_count'],
                'relcl': feats['relcl'],
                'relcl_count': feats['relcl_count'],
            }
...
def main():
    ...
    # 加载 spaCy 模型
    log(f"加载 spaCy 模型 en_core_web_trf...")
    import time
    ...
    try:
        ...
        nlp_trf = spacy.load('en_core_web_trf')
    log(f"spaCy 模型加载完成，耗时 {time.time() - _load_start:.2f}s")
    ...
    def process_one(persona):
        ...
        if query_data:
            ...
            result = {
                ...
                'relcl': query_data['relcl'],
                'relcl_count': query_data['relcl_count'],
                'elapsed': time.time() - start,
            }
            log(f"  [target={target_relcl_count}, used={used_relcl_types}] OK: relcl={query_data['relcl_count']}, words={query_data['word_count']}")
            return result
        else:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(process_one, p): p for p in personas}
    ...
    if OUTPUT_FILE:
        ...
        output_file = f'/fs04/ar57/wenyu/result/personal_query/06_query/persona_generated_queries.json'

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    # 统计成功率
    success_count = sum(1 for r in results if r.get('relcl_count') == r.get('target_relcl'))
    log(f"\n{'='*60}")
    log(f"Total: {len(results)} queries generated")
    log(f"Success: {success_count}/{len(results)} ({100*success_count/len(results):.1f}%)")

    # relcl_count 分布
    relcl_dist = {}
    for r in results:
        cnt = r.get('relcl_count', 0)
        relcl_dist[cnt] = relcl_dist.get(cnt, 0) + 1
    log(f"relcl_count分布: {relcl_dist}")
