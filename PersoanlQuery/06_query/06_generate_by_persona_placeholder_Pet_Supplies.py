...
USER_ERROR_FILE = '/home/wlia0047/ar57/wenyu/result/personal_query/04_writing_analysis/Pet_Supplies/acl_ccomp_error.json'
ACL_OUTPUT_FILE = '/fs04/ar57/wenyu/result/personal_query/06_query/Pet_Supplies/acl_query.json'
CCOMP_OUTPUT_FILE = '/fs04/ar57/wenyu/result/personal_query/06_query/Pet_Supplies/ccomp_query.json'
...
def build_acl_batch_prompt(attrs: dict, correct_words: list = None, groundtruth_level: int = None) -> tuple:
    ...
    attrs: 属性字典 {'A1': ..., 'A2': ..., 'A3': ..., 'A4': ..., 'A5': ...}
    correct_words: 用户的正确词列表，如果为 None 则不包含
    groundtruth_level: groundtruth 级别，如果为 None 则不包含
...
def main():
    ...
    def process_one_user(persona):
        ...
        # ========== ACL 批量查询（K=0,1,2,3）==========
        correct_words = [ep['corrected'] for ep in errors['acl'][:10] if ep.get('corrected')] if persona['has_acl_errors'] and errors and errors.get('acl') else None
        system_base, user_content = build_acl_batch_prompt(attrs, correct_words=correct_words, groundtruth_level=ground_truth_acl if persona['has_acl_errors'] and correct_words else None)
        ...
        # 构建最终结果
        for acl_level in range(4):
            if acl_level not in acl_results_dict:
                continue

            is_ground_truth = (acl_level == ground_truth_acl)
            entry = acl_results_dict[acl_level]
            query = entry['query']
        ...
        # ========== CCOMP 批量查询（K=0,1,2,3）==========
        correct_words = [ep['corrected'] for ep in errors['ccomp'][:10] if ep.get('corrected')] if persona['has_ccomp_errors'] and errors and errors.get('ccomp') else None
        system_base, user_content = build_ccomp_batch_prompt(attrs, correct_words=correct_words, groundtruth_level=ground_truth_ccomp if persona['has_ccomp_errors'] and correct_words else None)
        ...
        # 构建最终结果
        for ccomp_level in range(4):
            if ccomp_level not in ccomp_results_dict:
                continue

            is_ground_truth = (ccomp_level == ground_truth_ccomp)
            entry = ccomp_results_dict[ccomp_level]
            query = entry['query']
            attrs_used = entry.get('attrs_used', {})
            used_correct_words = entry.get('used_correct_words', [])
