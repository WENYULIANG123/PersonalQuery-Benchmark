...
# 构建 Prompt
# ========================================
def build_acl_prompt(query_level: int, attrs: dict) -> tuple:
    """为指定 ACL 版本构建 prompt
...
def build_ccomp_prompt(query_level: int, attrs: dict) -> tuple:
...
def prewarm_cache():
    ...
    # 预热 ACL cache
    if not _acl_cache_warmed:
        log("[Cache] 预热 ACL cache...")
        system, user = build_acl_prompt(0, generic_attrs)
        call_llm(user, system_base=system, is_acl=True)
        _acl_cache_warmed = True
        log("[Cache] ACL cache 预热完成")

    # 预热 CCOMP cache
    if not _ccomp_cache_warmed:
        log("[Cache] 预热 CCOMP cache...")
        system, user = build_ccomp_prompt(0, generic_attrs)
...
def main():
    ...
    def process_one_user(persona):
        ...
        for acl_level in range(4):
            ...
            # 直接用属性值生成查询
            system_base, user_content = build_acl_prompt(acl_level, attrs)
            response = call_llm(user_content, system_base=system_base, is_acl=True)

            # 解析结果
            result = parse_query_response(response, 'acl', acl_level)
            if not result or len(result) == 0:
                log(f"    [DEBUG] {query_key} 解析失败，user={uid}")
                continue
            ...
            if is_ground_truth and has_acl_err and errors and errors.get('acl'):
                correct_words = [ep['corrected'] for ep in errors['acl'][:10] if ep.get('corrected')]
                if correct_words:
                    # 简单替换第一个正确词（实际应用中可能需要更复杂的逻辑）
        ...
        for ccomp_level in range(4):
            ...
            # 直接用属性值生成查询
            system_base, user_content = build_ccomp_prompt(ccomp_level, attrs)
            response = call_llm(user_content, system_base=system_base, is_acl=False)

            # 解析结果
            result = parse_query_response(response, 'ccomp', ccomp_level)
            if not result or len(result) == 0:
                log(f"    [DEBUG] {query_key} 解析失败，user={uid}")
                continue
            ...
            # 如果是 ground_truth 且有错误，生成 noisy 版本
            if is_ground_truth and has_ccomp_err and errors and errors.get('ccomp'):
