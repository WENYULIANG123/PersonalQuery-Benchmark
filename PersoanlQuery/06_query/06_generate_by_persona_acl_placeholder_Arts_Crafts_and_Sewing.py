...
MAX_WORKERS = 10
BATCH_SIZE = 100
USER_PROFILES_FILE = '/home/wlia0047/ar57/wenyu/result/personal_query/05_syntactic_analysis/Arts_Crafts_and_Sewing/acl_user_profiles.json'
...
USER_ERROR_FILE = '/home/wlia0047/ar57/wenyu/result/personal_query/04_writing_analysis/Arts_Crafts_and_Sewing/acl_error.json'
OUTPUT_FILE = '/fs04/ar57/wenyu/result/personal_query/06_query/Arts_Crafts_and_Sewing/acl_query.json'
...
def call_llm(prompt: str) -> str:
    """调用LLM，遇到错误时自动重试
    - 429/529 错误（负载/速度限制）：无限重试
    - 其它错误：最多重试10次
    """
    client = MiniMaxAnthropicClient(model='MiniMax')
    wait_seconds = 5
    max_retries = 10  # 非429/529错误的重试上限
    ...
    while True:
        try:
            ...
            if text:  # 成功获取非空响应
                return text
            # 空响应也算错误，重试
            log(f"    [WARN] 空响应，重试 (attempt {attempt + 1})")
            ...
            is_rate_limit = '429' in error_str or '529' in error_str or 'rate' in error_str or 'limit' in error_str or 'overload' in error_str
            is_retryable = is_rate_limit or any(code in error_str for code in ['500', '502', '503', '504', '520', '522', '530', 'too many request'])
            ...
            if is_rate_limit:
                # 429/529错误无限重试
                log(f"    [WARN] 速率限制 ({e})，无限重试 (attempt {attempt + 1})")
                time.sleep(wait_seconds)
                wait_seconds += 5
                attempt += 1
                continue
                ...
                if attempt < max_retries - 1:
                    log(f"    [WARN] API错误 ({e})，重试 (attempt {attempt + 1}/{max_retries})")
                    time.sleep(wait_seconds)
                    wait_seconds += 5
                    attempt += 1
                else:
...
def main():
    ...
    # 先计算所有用户的 ground_truth_acl，过滤掉 > 3 的用户
    def compute_ground_truth_acl(uid):
        ...
        if words_per_acl and words_per_acl > 0:
            ...
            ground_truth_acl = 0
        return ground_truth_acl

    # 筛选目标用户（优先选有错误的用户，但必须在两个画像中都存在，且 ground_truth_acl <= 3）
    all_candidate_uids = list(profile_map.keys())  # 所有 acl 画像用户

    # 分别过滤出 error 和 normal 用户，同时检查 ground_truth_acl <= 3
    error_uids = []
    for uid in user_errors:
        if uid in profile_map and uid in user_wpa_map:
            g_acl = compute_ground_truth_acl(uid)
            if g_acl is not None and g_acl <= 3:
                error_uids.append(uid)

    normal_uids = []
    for uid in profile_map:
        if uid not in user_errors and uid in user_wpa_map:
            g_acl = compute_ground_truth_acl(uid)
            if g_acl is not None and g_acl <= 3:
                normal_uids.append(uid)
