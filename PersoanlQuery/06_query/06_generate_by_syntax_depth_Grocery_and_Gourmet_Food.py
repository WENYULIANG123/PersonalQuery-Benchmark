#!/usr/bin/env python3
"""
Generate Grocery_and_Gourmet_Food queries controlled by user-level syntax tree depth.

This script uses the Stage 5 user_average_syntax_depth.json output as the
target syntactic style signal. Each generated query must use exactly five
product attributes and must match the user's rounded dependency-tree depth.
"""
...
BASE_SCRIPT = Path("/home/wlia0047/ar57/wenyu/PersoanlQuery/06_query/06_generate_by_persona_placeholder_Grocery_and_Gourmet_Food.py")
if not BASE_SCRIPT.exists():
    raise FileNotFoundError(f"base script not found: {BASE_SCRIPT}")

_BASE_SPEC = importlib.util.spec_from_file_location("stage6_grocery_base", BASE_SCRIPT)
...
) / CATEGORY / "query_by_syntax_depth.json"
...
def compute_sentence_syntax_tree_depth(sentence: str) -> int:
    ...
    doc = _load_spacy_model()(sentence)
    depths = []
    for token in doc:
...
def build_syntax_depth_prompt(
    ...
    _ = target_depth, avg_depth, candidate_count
...
def process_one_user(task: dict) -> dict | None:
    ...
    for idx, parsed in enumerate(candidates, start=1):
        ...
        actual_depth = compute_sentence_syntax_tree_depth(query)
        if actual_depth != target_depth:
            rejection_reasons.append(
                f"candidate={idx}: depth target={target_depth}, actual={actual_depth}, query={query}"
            )
            continue
    ...
    log(f"    [ERROR] No strict valid candidate, user={uid}, candidates={len(candidates)}")
