#!/usr/bin/env python3
"""Search one shared PCA threshold with >=30% per side and max overall two-tier gap."""
...
SUMMARY_FILE = OUTPUT_DIR / "global_binary_pca_threshold_min30_search.json"
MIN_TIER_RATIO = 0.30
...
def oriented_pca_scores(feature_rows: list[dict]) -> dict[tuple[str, str], float]:
    ...
    if corr < 0:
        scores = -scores
    return {
        (row["user_id"], row["asin"]): float(score)
...
def build_aligned_rows(score_index: dict[tuple[str, str], float], retriever_index: dict[str, dict[tuple[str, str], float]]) -> tuple[list[dict], list[str]]:
    ...
    rows = []
    for key in retriever_index[first_retriever]:
...
def search_best_threshold(rows: list[dict], retrievers: list[str]) -> dict:
    ...
    for split_idx in range(min_tier_size, n - min_tier_size + 1):
        if split_idx == n:
            continue
        threshold = float(score_array[split_idx - 1])
        next_score = float(score_array[split_idx])
        if threshold == next_score:
            continue
        checked += 1
        low_count = split_idx
        ...
        candidate = {
            "threshold": threshold,
            "next_score_after_threshold": next_score,
            "low_count": low_count,
            "high_count": high_count,
            "imbalance": int(abs(low_count - high_count)),
    ...
    if best is None:
        raise ValueError("在 >=30% 约束下没有可用阈值")
...
def main() -> None:
    ...
    score_index = oriented_pca_scores(feature_rows)
    ...
    summary = {
        ...
        "objective": "max_total_abs_gap_with_each_side_at_least_30_percent",
        "best_threshold": best,
    }
