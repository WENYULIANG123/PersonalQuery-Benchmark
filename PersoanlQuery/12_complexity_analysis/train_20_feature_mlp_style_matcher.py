#!/usr/bin/env python3
"""Train a small MLP to identify true query-user style matches from 20 syntax features."""
...
SUMMARY_FILE = INPUT_DIR / "twenty_feature_mlp_style_matcher_summary.json"
PAIR_RECORD_FILE = INPUT_DIR / "twenty_feature_mlp_style_matcher_test_pairs.jsonl"
...
def build_pairs(
    ...
    return np.vstack(x_rows).astype(np.float32), np.array(y_rows, dtype=np.float32), pair_records
...
def train_model(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray) -> tuple[StyleMLP, list[dict], int]:
    model = StyleMLP(input_dim=x_train.shape[1])
    ...
    if best_state is None:
        raise ValueError("训练未产生 best_state")
...
def main() -> None:
    ...
    query_rows, query_matrix, groups = build_query_vectors(raw_query_rows, feature_names, scaler, review_vectors)
    ...
    log(f"train/val/test user 数: {len(train_users)}/{len(val_users)}/{len(test_users)}")
    ...
    x_val, y_val, _ = build_pairs(
        [query_rows[idx] for idx in val_idx],
        query_matrix[val_idx],
        review_vectors,
        val_users,
        rng,
    )
    ...
    log("开始训练 MLP style matcher")
    model, history, best_epoch = train_model(x_train, y_train, x_val, y_val)

    train_scores = predict_scores(model, x_train)
    val_scores = predict_scores(model, x_val)
    test_scores = predict_scores(model, x_test)
    train_eval = evaluate_scores(y_train, train_scores)
