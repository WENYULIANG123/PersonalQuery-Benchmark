#!/usr/bin/env python3
"""Run a supervised 1D latent-score experiment for Baby_Products bm25 hit@10."""
...
SUMMARY_FILE = OUTPUT_DIR / "bm25_supervised_latent_3tier_summary.json"
ROW_FILE = OUTPUT_DIR / "bm25_supervised_latent_3tier_records.jsonl"
...
class SupervisedLatentNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return latent.squeeze(-1), logits
...
def build_matrices(feature_names: list[str], rows: list[dict], user_splits: dict[str, set[str]]) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, list[dict]]]:
    ...
    for split_name, items in split_rows.items():
        ...
        labels = {int(row["hit_at10"]) for row in items}
        if labels != {0, 1}:
            raise ValueError(f"{split_name} split 不同时包含 0/1 标签")
    ...
    for split_name, items in split_rows.items():
        ...
        split_labels[split_name] = np.array([float(row["hit_at10"]) for row in items], dtype=np.float32)
    return split_features, split_labels, split_rows


def evaluate_loss(model: nn.Module, features: np.ndarray, labels: np.ndarray, criterion: nn.Module, device: torch.device) -> float:
    model.eval()
    with torch.no_grad():
...
def train_model(split_features: dict[str, np.ndarray], split_labels: dict[str, np.ndarray]) -> tuple[SupervisedLatentNet, list[dict]]:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = SupervisedLatentNet(input_dim=split_features["train"].shape[1]).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    criterion = nn.BCEWithLogitsLoss()
    train_inputs = torch.from_numpy(split_features["train"]).to(device)
    train_targets = torch.from_numpy(split_labels["train"]).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        model.train()
        optimizer.zero_grad()
        _, logits = model(train_inputs)
        loss = criterion(logits, train_targets)
        if not torch.isfinite(loss):
            raise ValueError(f"epoch {epoch} 出现非有限训练损失")
        loss.backward()
        optimizer.step()

        train_loss = float(loss.item())
        val_loss = evaluate_loss(model, split_features["val"], split_labels["val"], criterion, device)
        history.append({
            "epoch": epoch,
    ...
    if best_state is None:
        raise ValueError("训练结束后没有保存到最优模型")
    model.load_state_dict(best_state)
    return model, history


def extract_latent_scores(model: SupervisedLatentNet, split_features: dict[str, np.ndarray], split_labels: dict[str, np.ndarray]) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, float]]:
    ...
    for split_name, features in split_features.items():
        ...
        with torch.no_grad():
            ...
            prob_outputs[split_name] = torch.sigmoid(logits).cpu().numpy()

    corr = np.corrcoef(score_outputs["train"], split_labels["train"])[0, 1]
    if np.isnan(corr):
        raise ValueError("训练集 latent score 与标签相关性为 NaN")
    if corr < 0:
    ...
    metrics = {
        "train_score_label_corr": float(corr),
        "test_auc": float(roc_auc_score(split_labels["test"], prob_outputs["test"])),
        "test_average_precision": float(average_precision_score(split_labels["test"], prob_outputs["test"])),
    }
    return score_outputs, prob_outputs, metrics
...
def main() -> None:
    ...
    split_features, split_labels, split_rows = build_matrices(feature_names, rows, user_splits)
    model, history = train_model(split_features, split_labels)
    score_outputs, prob_outputs, model_metrics = extract_latent_scores(model, split_features, split_labels)
    thresholds = compute_train_thresholds(score_outputs["train"])
    output_rows, test_summary = build_test_summary(split_rows, score_outputs, prob_outputs, thresholds)
