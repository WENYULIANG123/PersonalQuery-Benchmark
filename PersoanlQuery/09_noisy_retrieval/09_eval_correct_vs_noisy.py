#!/usr/bin/env python3
"""
简化评估脚本 - 评估 correct vs noisy 查询的整体性能
不对 ACL/CCOMP 等级进行分组，只评估总体 P@10
"""
...
def load_queries(query_type: str = 'correct') -> Tuple[List[Dict], Dict]:
    """加载查询数据，不区分 ACL/CCOMP 等级

    Args:
        query_type: 'correct' 或 'noisy'

    Returns:
        queries: 查询列表，每项包含 user_id, asin, query, query_category, level
        doc_ids_set: 所有相关文档 ASIN 集合
    """
    queries = []
    doc_ids_set = set()

    if query_type == 'correct':
        # 从主 query.json 加载 correct 查询
        ...
        # 从 07_inject_noisy 目录加载 noisy 查询
        for category, noisy_file in [('acl', ACL_NOISY_QUERY_FILE), ('ccomp', CCOMP_NOISY_QUERY_FILE)]:
            ...
            for item in data:
                ...
                if noisy_query:
                    query_text = noisy_query.get('query', '')
                    level = noisy_query.get('level', 0)
                    word_count = noisy_query.get('word_count', 0)
                    if query_text:
                        queries.append({
                            'user_id': user_id,
                            'asin': asin,
...
def main():
    log("=" * 60)
    log(f"简化评估 - Correct vs Noisy 查询")
