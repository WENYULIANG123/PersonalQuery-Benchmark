...
def main():
    ...
    log(f"有错误模式的任务数: {tasks_with_errors}")
    ...
    with ThreadPoolExecutor(max_workers=EFFECTIVE_MAX_WORKERS) as executor:
        futures = {executor.submit(process_noisy_query_task, task, NOISY_USER_CONTENT_TEMPLATE, INJECT_ERROR_COUNT, NOISY_SYSTEM_BASE, idx): (task, idx) for idx, task in enumerate(tasks)}
        for processed_count, future in enumerate(as_completed(futures), start=1):
            ...
            # 只有真正成功注入错误的才保存（query_rewritten=True）
            if status == 'success' and result.get('query_rewritten'):
                success_count += 1
                all_results.append(result)  # 收集结果
                write_json_array(all_results, NOISY_OUTPUT_FILE)  # 每完成一个就写入
