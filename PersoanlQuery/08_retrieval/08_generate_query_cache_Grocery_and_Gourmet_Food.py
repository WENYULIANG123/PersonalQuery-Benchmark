...
PERSONA_GENERATED_QUERIES_FILE = "/home/wlia0047/ar57/wenyu/result/personal_query/06_query/Grocery_and_Gourmet_Food/ccomp_query.json"
...
def generate_cache_from_persona_source(retriever_names: Optional[List[str]] = None, clear_cache_before: bool = False):
    """从 ccomp_query.json 生成 correct 和 noisy 两套查询缓存
    ...
    if not correct_queries and not noisy_queries:
        log_with_timestamp("⚠️  没有从 ccomp_query.json 加载到任何查询")
    ...
    log_with_timestamp("🚀 开始生成查询缓存 (ccomp_query.json - correct + noisy)")
    log_with_timestamp("=" * 80)
