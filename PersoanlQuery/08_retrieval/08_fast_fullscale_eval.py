...
QUERY_CACHE_BASE_DIR = "/home/wlia0047/ar57_scratch/wenyu/result/personal_query/12_retrieval/query_cache"
QUERY_TYPES = ['correct', 'noisy']  # 两种查询类型
QUERIES_FILE = "/home/wlia0047/ar57/wenyu/result/personal_query/06_query/ccomp_query.json"
    ...
    for user_idx, user_id in enumerate(matched_users):
        ...
        for i, (retrieved, relevant_asin) in enumerate(zip(results, query_asins)):
            ...
            all_query_records.append({
                ...
                'ccomp_ratio': query_ccomp_ratios[i],
...
def print_ccomp_table(all_results: List[Dict], k_values: List[int]):
    log("\n" + "=" * 120)
    log(f"{GROUP_FIELD.upper()} 分组评估结果")
    log("=" * 120)
...
def print_cross_tab(all_results: List[Dict], metric: str):
    log(f"\n{'='*80}")
    log(f"交叉对比表: {metric}")
    log(f"{'='*80}")
...
def run_ols_ccomp_analysis(all_results: List[Dict]):
    ...
    log("=" * 80)
    log("目的: 在控制 mean_idf + query_length + ccomp_ratio 后，检验 ccomp_high 的净效应")
    log("")
    ...
    df['ccomp_high'] = (df['ccomp'] >= threshold).astype(int)
    log(f"  分组字段: {GROUP_FIELD}, 高低分界阈值: {threshold}")
    log(f"  有效分组: {UNIQUE_GROUPS}")

    log(f"  总样本数: {len(df)} queries")
    log(f"  ccomp_high=1: {df['ccomp_high'].sum()}, ccomp_high=0: {(df['ccomp_high']==0).sum()}")
    ...
    for retriever in sorted(df['retriever'].unique()):
        ...
        formula = 'p_at10 ~ ccomp_high + mean_idf + query_length + ccomp_ratio'

        try:
            df_r_clean = df_r[['p_at10', 'ccomp_high', 'mean_idf', 'query_length', 'ccomp_ratio']].dropna()
            ...
            p_val = model.pvalues.get('ccomp_high', float('nan'))
            coef = model.params.get('ccomp_high', float('nan'))
            t_val = model.tvalues.get('ccomp_high', float('nan'))
    ...
    # 汇总 OLS
    formula_pooled = 'p_at10 ~ ccomp_high + mean_idf + query_length + ccomp_ratio + C(retriever)'
    try:
        df_clean = df[['p_at10', 'ccomp_high', 'mean_idf', 'query_length', 'ccomp_ratio', 'retriever']].dropna()
        model_pooled = smf.ols(formula_pooled, data=df_clean).fit()
        p_val = model_pooled.pvalues.get('ccomp_high', float('nan'))
        coef = model_pooled.params.get('ccomp_high', float('nan'))
        t_val = model_pooled.tvalues.get('ccomp_high', float('nan'))
        sig = "***" if not np.isnan(p_val) and p_val < 0.001 else \
              "**"  if not np.isnan(p_val) and p_val < 0.01  else \
              "*"   if not np.isnan(p_val) and p_val < 0.05  else ""
        ols_table.append({
...
def run_paired_difference_analysis(all_results: List[Dict]):
    ...
    log(f"  配对样本数: {len(pivot)} ({n_ret}检索器 × {pairs_per_ret}对/检索器，每对 = 同用户ccomp3 vs ccomp0)")
...
def print_query_type_comparison(all_results_by_type: Dict[str, List[Dict]], k_values: List[int]):
    """打印 correct vs noisy 总体性能比较表（不按 ACL 分组）"""
    log("\n" + "=" * 80)
    log("CORRECT vs NOISY 总体性能比较")
    log("=" * 80)
    ...
    for retriever in retrievers:
        ...
        diffs = []
        for qt in QUERY_TYPES:
            qt_results = all_results_by_type.get(qt, [])
            r = next((x for x in qt_results if x['retriever'] == retriever), None)
            if r:
                p10 = r['metrics'].get('P@10', 0.0)
                row += f" {p10:.4f}     "
                diffs.append(r['metrics'].get('P@10', 0.0))
            else:
                row += " N/A       "
                diffs.append(None)

        if len(diffs) == 2 and all(d is not None for d in diffs):
...
def main():
    ...
    if all_results_by_type.get('correct'):
        for metric in ['P@1', 'P@3', 'P@5', 'P@10', 'N@10', 'MR@10', 'H@10']:
            print_cross_tab(all_results_by_type['correct'], metric)

    # ========== CORRECT vs NOISY 比较 ==========
    # 注意：目前 noisy 版本只有 ground truth (acl=0) 的 100 个查询，
    # acl=1,2,3 没有 noisy 查询，所以 CORRECT vs NOISY 比较只能针对 acl=0，
    # 且无法做配对检验。
